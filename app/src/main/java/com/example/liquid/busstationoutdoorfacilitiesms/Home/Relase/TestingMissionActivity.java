...
public class TestingMissionActivity extends AppCompatActivity implements View.OnClickListener {

    private Spinner mTestSpinnerDirection;
    private Spinner mTestType;
    /**
     * 发布任务
     */
    private Button mBtFinish;
    private Spinner mTestSpinnerStation;
    /**
     * -----------选择起始日期~-----------
     */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_testing_mission);
        initView();
        String[] ctype = new String[]{"茶汤桥", "七古登", "湖州街上塘路口"};
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, ctype);  //创建一个数组适配器
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);     //设置下拉列表框的下拉选项样式
        Spinner spinner = super.findViewById(R.id.test_spinner_station);
    ...
    private void initView() {
        mTestSpinnerDirection = (Spinner) findViewById(R.id.test_spinner_direction);
        mTestType = (Spinner) findViewById(R.id.test_type);

        mBtFinish = (Button) findViewById(R.id.bt_finish);
        mBtFinish.setOnClickListener(this);
        mTestSpinnerStation = (Spinner) findViewById(R.id.test_spinner_station);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            default:
                break;
            case R.id.bt_finish:
                MessageDB messageDB = new MessageDB(mTestType.getSelectedItem().toString(), mTestSpinnerStation.getSelectedItem().toString(),
                        mTestSpinnerDirection.getSelectedItem().toString(), "", "未接收",
                ...
                MessageFragment.MessageItemArrayList.add(new MessageItem(MessageFragment.MessageItemArrayList.size() + 1,
                        mTestType.getSelectedItem().toString(), "", mTestSpinnerStation.getSelectedItem().toString(),  mTestSpinnerDirection.getSelectedItem().toString(), "未接收", R.mipmap.unreceived, R.mipmap.one));

                Toast.makeText(this, "检测任务发布完成！", Toast.LENGTH_SHORT).show();
