...
public class NextMissionActivity extends AppCompatActivity implements View.OnClickListener, AdapterView.OnItemSelectedListener {

    private Spinner mSpinnerNextStation;
    private Spinner mSpinnerNextDirection;
    private Spinner mSpinnerNextBrand;
    private Spinner mSpinnerNextStaff;
    ...
    private void initView() {
        mSpinnerNextStation = (Spinner) findViewById(R.id.spinner_next_station);
        String[] ctype = new String[]{"茶汤桥", "七古登", "湖州街上塘路口"};
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, ctype);  //创建一个数组适配器
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);     //设置下拉列表框的下拉选项样式
        Spinner spinner = super.findViewById(R.id.spinner_next_station);
        spinner.setAdapter(adapter);
        mSpinnerNextStation.setOnItemSelectedListener(this);

        mSpinnerNextDirection = (Spinner) findViewById(R.id.spinner_next_direction);
        mSpinnerNextDirection.setOnItemSelectedListener(this);
        mSpinnerNextBrand = (Spinner) findViewById(R.id.spinner_next_brand);
        mSpinnerNextBrand.setOnItemSelectedListener(this);
        mSpinnerNextStaff = (Spinner) findViewById(R.id.spinner_next_staff);
        mSpinnerNextStaff.setOnItemSelectedListener(this);
        ...
        mImageReturn = (ImageView) findViewById(R.id.image_return);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            default:
                break;
            case R.id.bt_finish:
                MessageDB messageDB = new MessageDB("下刊", mSpinnerNextStation.getSelectedItem().toString(),
                        mSpinnerNextDirection.getSelectedItem().toString(), mSpinnerNextBrand.getSelectedItem().toString(), "未接收",
                ...
                MessageFragment.MessageItemArrayList.add(new MessageItem(MessageFragment.MessageItemArrayList.size() + 1,
                        "下刊", brand, station, direction, "未接收", R.mipmap.unreceived, R.mipmap.one));

                Toast.makeText(this, "下刊任务发布完成！", Toast.LENGTH_SHORT).show();
                finish();
                break;
...
//                mDateEnd.setText("当前时间："+ Year[0] +"年"+ month[0] +"月"+ day[0] +"日   "+i+":"+i1);
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        switch (parent.getId()) {
            case R.id.spinner_next_station:
                station = (String) mSpinnerNextStation.getSelectedItem();
                break;

            case R.id.spinner_next_direction:
                direction = (String) mSpinnerNextDirection.getSelectedItem();
                break;

            case R.id.spinner_next_brand:
                brand = (String) mSpinnerNextBrand.getSelectedItem();
                break;

            case R.id.spinner_next_staff:
                staff = (String) mSpinnerNextStaff.getSelectedItem();
                break;
        }
